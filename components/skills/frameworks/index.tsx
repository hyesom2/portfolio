import { FRAMEWORKS_DATA } from '@/data/skillsData';
import Image from 'next/image';

export default function Frameworks() {
  return (
    <section>
      <h2 className='text-[32px] text-gray-500 font-bold mb-5'>
        프레임워크 및 라이브러리
      </h2>
      <ul className='grid grid-cols-1 gap-6 md:grid-cols-2'>
        {FRAMEWORKS_DATA.map(({ id, title, content, img_url }) => (
          <li
            key={id}
          ...
          >
            ...
            <div className='flex flex-col justify-start items-start gap-2 md:flex-row md:items-center'>
              <h3 className='shrink-0 font-bold text-[18px] md:w-[120px] md:text-[20px]'>
                {title}
              </h3>
